...
<script>
/* =====================================================
   Ajin Music Player — GitHub API 자동 트랙 로딩
   MP3 파일을 레포에 업로드하면 자동으로 플레이리스트에 추가
   ===================================================== */
const REPO   = 'ajinnetworks/Ajin-Soft-Rock-Ballad';
const BRANCH = 'main';
...
/* 제외할 파일 패턴 (비MP3, 커버 이미지 등) */
const SKIP = ['cover', 'index', 'manifest', 'sw'];
...
function togglePlay(){
  ...
  playing=!playing;
  if(playing){if(!aud.src){aud.src=tracks[ci].url;aud.load();}aud.play().catch(()=>{});startVU();}
  else{aud.pause();stopVU();}
...
async function loadTracksFromGitHub(){
  setSt('트랙 목록 로딩 중...','ld');
  try{
    /* titles.json + 파일 목록 병렬 로드 */
    const [filesRes, titlesRes] = await Promise.all([
      fetch(API),
      fetch(RAW+'titles.json?t='+Date.now())
    ]);

    const files = filesRes.ok ? await filesRes.json() : [];
    const titleMap = titlesRes.ok ? await titlesRes.json() : {};
    ...
    if(mp3files.length === 0){
      setSt('MP3 파일 없음 — 레포에 업로드해 주세요','er');
      return;
    }

    /* titles.json 매핑 우선 → 없으면 파일명 자동 변환 */
    tracks = mp3files.map(f => {
      const meta = titleMap[f.name] || {};
      return {
        title:  meta.title  || toTitle(f.name),
        artist: meta.artist || 'Ajin Soft Rock Ballad',
        year:   meta.year   || '2025 · Soft Rock Ballad',
        dur:    '--:--',
        secs:   0,
        url:    f.download_url,
    ...
    setSt('로드 실패: '+e.message,'er');
    tracks=[
      {title:'Yesterday Once More',artist:'The Carpenters',year:'1973 · Soft Rock',dur:'--:--',secs:0,url:RAW+'Carpenters-Yesterday-Once-More.mp3',cover:COVER},
      {title:'Good Day',artist:'Ajin Soft Rock Ballad',year:'2025 · Korean Ballad',dur:'--:--',secs:0,url:RAW+'good-day.mp3',cover:COVER},
    ];
    buildTrackSelect();
    loadT(0, false);
  }
