...
<div class="music-page">
  ...
  <div class="player-wrap">
    ...
    <div class="player-body">
      ...
      <div id="play-hint" style="display:none;color:#ff6b6b;font-size:12px;text-align:center;padding:4px 18px;font-family:'Space Mono',monospace">화면을 터치한 후 재생 버튼을 눌러주세요</div>
      <div class="vol-row">
      ...
      <div class="panel" id="panelAdd" style="display:none">
        <div class="add-panel">
          <div>
            ...
            <select class="add-inp" id="trackSel" style="width:100%;cursor:pointer"></select>
          </div>
          <div>
          ...
          <div>
            <div class="add-lbl">앨범 커버 이미지 URL</div>
            <div class="add-row">
              <input class="add-inp" id="coverUrl" placeholder="https://raw.githubusercontent.com/...jpg">
...
<script>
/* =====================================================
   Ajin Music Player — 하드코딩 트랙 목록
   ===================================================== */
const RAW   = 'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/';
const COVER = RAW + 'cover-ajin.jpg';

/* 트랙 목록 직접 정의 (GitHub API 불필요) */
const TRACKS = [
  {
    url:   'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/Carpenters-Yesterday-Once-More.mp3',
    title: 'Yesterday Once More',
    artist:'The Carpenters',
    year:  '1973',
    genre: 'Soft Rock',
    cover: 'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/cover-ajin.jpg'
  },
  {
    url:   'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/Jim-Croces-Time-in-a-Bottle.mp3',
  ...
  {
    url:   'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/The-Light-Left-at-Dawn25.mp3',
    title: 'The Light Left at Dawn',
    artist:'Ajin Soft Rock',
    year:  '2025',
    genre: 'Soft Rock Ballad',
    cover: 'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/cover-ajin.jpg'
  },
  {
    url:   'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/Where-Silence-Holds-Your-Name25.mp3',
    title: 'Where Silence Holds Your Name',
    artist:'Ajin Soft Rock',
    year:  '2025',
    genre: 'Soft Rock Ballad',
    cover: 'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/cover-ajin.jpg'
  ...
  {
    url:   'https://raw.githubusercontent.com/ajinnetworks/Ajin-Soft-Rock-Ballad/main/good-day.mp3',
    title: 'Good Day',
    artist:'Ajin Soft Rock',
    year:  '2025',
    genre: 'Soft Rock Ballad',
...
function animVU(){
  if(!playing){vbs.forEach(b=>{b.style.height='3px';b.style.background='#1a1a1e';});return;}
  const base=Math.random()*0.5+0.35;
  vbs.forEach((b,i)=>{
    const w=Math.sin(i/24*Math.PI)*0.6+0.4;
    const h=Math.max(3,Math.round((base*w+Math.random()*0.2)*26));
    b.style.height=h+'px';b.style.background=vuC(h/26);
  });
}
function fmt(s){if(!s||s<=0)return '--:--';return Math.floor(s/60)+':'+String(Math.floor(s%60)).padStart(2,'0');}
function setSt(msg,type=''){
  const el=document.getElementById('statusBar');
  el.innerHTML=type?'· '+msg:'<span class="live-dot"></span>'+msg;
  el.className='status-bar'+(type?' '+type:'');
}
function showTab(n,btn){
  document.getElementById('panelPl').style.display=n==='pl'?'block':'none';
  document.getElementById('panelAdd').style.display=n==='add'?'block':'none';
  document.querySelectorAll('.tab-btn').forEach(t=>t.classList.remove('ac'));
...
function updateUI(){
  ...
  document.getElementById('tYear').textContent=t.year;
  document.getElementById('covNum').textContent=String(ci+1).padStart(2,'0')+' / '+String(tracks.length).padStart(2,'0');
  document.getElementById('vinyl').className='vinyl'+(playing?' sp':'');
  const img=document.getElementById('covImg');
  img.onload=()=>{img.style.display='block';document.getElementById('covPh').style.display='none';};
  img.onerror=()=>{img.style.display='none';document.getElementById('covPh').style.display='flex';};
  img.src=COVER+'?t='+Date.now();
  renderPL();
}
function renderPL(){
  document.getElementById('playlist').innerHTML=tracks.map((t,i)=>`
    <div class="pl-item${i===ci?' ac':''} has-url" onclick="selT(${i})">
      <span class="pl-num">${i===ci&&playing?'<span class="eq"><span></span><span></span><span></span></span>':String(i+1).padStart(2,'0')}</span>
      <div class="pl-body"><div class="pl-title">${t.title}</div><div class="pl-artist">${t.artist}</div></div>
      <span class="pl-dur">▶</span>
    </div>`).join('');
}
function updateProg(){
  const t=tracks[ci];const pct=t.secs>0?Math.min(100,curSec/t.secs*100):0;
  const p=pct.toFixed(1)+'%';
  document.getElementById('progFill').style.width=p;
  document.getElementById('progThumb').style.left=p;
  document.getElementById('curT').textContent=fmt(curSec);
}

aud.addEventListener('timeupdate',()=>{if(!aud.paused){curSec=aud.currentTime;updateProg();}});
aud.addEventListener('loadedmetadata',()=>{
  const d=Math.floor(aud.duration);
  tracks[ci].secs=d;tracks[ci].dur=fmt(d);
  document.getElementById('totT').textContent=fmt(d);
  setSt('재생 준비 완료 — '+tracks[ci].title,'ok');
  renderPL();
});
aud.addEventListener('ended',()=>nextT());
aud.addEventListener('error',()=>setSt('로드 실패 — 파일 확인 필요','er'));
aud.addEventListener('waiting',()=>setSt('버퍼링 중...','ld'));
aud.addEventListener('playing',()=>setSt('재생 중 — Ajin Soft Rock Ballad'));

function startVU(){clearInterval(vuInt);vuInt=setInterval(animVU,85);}
function stopVU(){clearInterval(vuInt);animVU();}

function loadT(idx, auto){
  if(!tracks.length) return;
  ci=idx;curSec=0;aud.pause();aud.src='';
  const t=tracks[idx];
  document.getElementById('totT').textContent='--:--';
  updateProg();
  aud.src=t.url;
  aud.volume=parseInt(document.getElementById('volVal').textContent)/100;
  aud.load();
  if(auto) aud.play().catch(()=>{});
  setSt('로딩 중 — '+t.title,'ld');
  updateUI();
}
function togglePlay(){
  if(!tracks.length){setSt('트랙 없음','er');return;}
  try{
    if(aud.paused){
      playing=true;
      if(!aud.src){aud.src=tracks[ci].url;aud.load();}
      const playPromise=aud.play();
      if(playPromise!==undefined){
        playPromise
          .then(()=>{startVU();updatePlayBtn();updateUI();})
          .catch(err=>{
            console.warn('재생 실패 (사용자 터치 필요):',err);
            playing=false;updatePlayBtn();showPlayHint();
          });
      }
    }else{
      playing=false;
      aud.pause();stopVU();updatePlayBtn();updateUI();
    }
  }catch(e){console.error('togglePlay 오류:',e);}
}
...
function copyLink(){
  navigator.clipboard.writeText('https://blog.ajinnetworks.co.kr/music/')
    ...
    .catch(()=>setSt('수동으로 복사해 주세요','er'));
}

/* =====================================================
   트랙 로딩 (하드코딩 — GitHub API 불필요)
   ===================================================== */
function loadTitles(){
  return Promise.resolve({});
}

function loadTracksFromGitHub(){
  try{
    ...
    setSt(tracks.length+'개 트랙 로드됨','ok');
    document.getElementById('covNum').textContent='01 / '+tracks.length;
    aud.volume=0.8;
    buildTrackSelect();
    loadT(0,false);
    console.log('트랙 '+tracks.length+'개 로드 완료');
  }catch(e){
    console.error('트랙 로드 오류:',e);
    setSt('트랙 로드 실패','er');
  }
}
...
function addAudio(){
  ...
  if(!url){setSt('URL을 입력해 주세요','er');return;}
  tracks[idx].url=url;
  setSt(tracks[idx].title+' MP3 등록 완료','ok');
  renderPL();
...
function addCover(){
  ...
  tracks[idx].cover=url;
  if(ci===idx){
    const img=document.getElementById('covImg');
    img.onload=()=>{img.style.display='block';document.getElementById('covPh').style.display='none';setSt(tracks[idx].title+' 커버 등록 완료','ok');};
    img.onerror=()=>{img.style.display='none';document.getElementById('covPh').style.display='flex';setSt('이미지 로드 실패','er');};
    img.src=url;
  } else {
